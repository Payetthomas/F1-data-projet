...
- Valeurs manquantes : `\N` remplacées par `NaN`, lignes supprimées après jointure
- Couverture temporelle : 1950 → 2024 (données post-2000 plus complètes)
- Distribution de `positionOrder` : uniforme de 1 à 20
- Dataset après nettoyage : **26 726 lignes propres**

## Biais et limites

- Les données météo historiques (Open-Meteo) ne sont disponibles qu'à partir de 1940 — toutes nos courses sont couvertes
- Les pilotes et écuries récents (2025-2026) ont des `driverId`/`constructorId` absents du dataset Kaggle
- La météo est la météo générale du jour, pas la météo exacte pendant la course